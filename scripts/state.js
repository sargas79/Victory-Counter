 ...
 * counts to a target under the progress rules, but the GM may name individual
 * numbers on the way: a named step means something on that number alone, and
 * the numbers around it mean nothing in particular.
 *
 ...
 *                          ordinal index as a string on a progress or step track.
...
/**
 * A named step on a step track. Structurally identical to a {@link Threshold},
 * because it is the same row of fields — the difference is in how it is read:
 * a step label means something on its own number, a threshold rung owns every
 * number up to the next one.
 * @typedef {Threshold} StepLabel
 */
 ...
 * @property {StepLabel[]} steps          The named steps, ascending by value. Steps mode.
 * @property {string|null} step           Id of the label sitting exactly on `current`, or null.
 * @property {boolean} announceSteps      Whether reaching a labelled step announces in chat.
 * @property {boolean} revealSteps        Whether players see the names of unreached steps.
...
/**
 * Sanitize a raw list of rungs: clamp each value into range, drop anything past
 * the cap, drop duplicate values, and sort ascending.
 *
 * Shared by the threshold ladder and the step labels, which are the same row of
 * fields differing only in how many are allowed and what range their values may
 * occupy. Written once so the dedupe, id and sort rules cannot drift apart: a
 * fix to how one list handles a duplicate is a fix to both.
 *
 * Two rungs on the same number are dropped down to one because only one of them
 * could ever own that number — keeping both would make which description the
 * players see depend on array order. The first one written wins, and the clamp
 * runs *before* the dedupe so two rows that clamp onto the same value collapse
 * the same way two rows typed on it would.
 *
 * The ascending sort is what lets {@link resolveBand} stop walking early, and it
 * is also the order the GM reads the list in.
 *
 * @param {any} raw
 * @param {{cap: number, floor: number, ceiling: number}} bounds
 * @returns {Threshold[]}
 */
function sanitizeRungs(raw, { cap, floor, ceiling }) {
  const list = Array.isArray(raw) ? raw : [];
  const byValue = new Map();
  const seenIds = new Set();

  for (const entry of list) {
    if (byValue.size >= cap) break;
    const rung = sanitizeThreshold(entry);
    rung.value = clampInt(rung.value, floor, ceiling);
...
/**
 * Sanitize a raw threshold ladder. Unchanged in behaviour: the bounds passed
 * here are the ones {@link sanitizeThreshold} already clamps to, so the extra
 * clamp inside the shared walk is a no-op for this list.
 * @param {any} raw
 * @returns {Threshold[]}
 */
export function sanitizeThresholds(raw) {
  return sanitizeRungs(raw, {
    cap: LIMITS.MAX_THRESHOLDS,
    floor: LIMITS.MIN_VALUE,
    ceiling: LIMITS.MAX_VALUE
  });
}

/**
 * Sanitize a raw list of step labels.
 *
 * The floor is 1, not 0: step 0 is the empty track, the state every step track
 * starts in, and a label there could never be "reached". The ceiling is
 * MAX_TARGET because a step label names a step, and a track cannot have more
 * steps than the largest target it may be given.
 *
 * @param {any} raw
 * @returns {StepLabel[]}
 */
export function sanitizeSteps(raw) {
  return sanitizeRungs(raw, {
    cap: LIMITS.MAX_STEP_LABELS,
    floor: 1,
    ceiling: LIMITS.MAX_TARGET
  });
}
...
export function sanitizeTrack(raw) {
  ...
  // The step fields are normalized in every mode too, and for the same reason:
  // a track flipped to another mode keeps the labels the GM wrote, and the panel
  // can report how many there are without the mode deciding whether the field is
  // trustworthy.
  ...
  if (merged.mode === TRACK_MODES.THRESHOLD) {
    ...
    merged.step = null;
  } else if (merged.mode === TRACK_MODES.STEPS) {
    // Progress rules: a step track counts up from an empty track to its target
    // and completes there, so it is bounded exactly as a progress track is.
    merged.current = clampInt(merged.current, 0, LIMITS.MAX_COUNT);
    merged.band = null;
    // Derived and stored for the same reasons as `band` above — with the one
    // difference that makes this mode its own: the match is exact, so a value
    // between two labels carries no label at all.
    merged.step = resolveStep(merged.current, merged.steps)?.id ?? null;
  } else {
 ...
 *
 * A step track does have a finish line: its last step is the target, and it
 * completes there like any other progress track. It therefore wants the rule
 * below unchanged, which is why the guard names threshold mode rather than
 * excluding progress mode.
 *
 * @param {Track} t
 * @returns {string}
 */
...
/**
 * Whether a track counts in discrete, individually nameable steps.
 * @param {Track} track
 * @returns {boolean}
 */
 ...
 * has allowed overshoot. Step tracks take that branch unchanged: they are
 * progress tracks that happen to be drawn as pips, and the Allow Progress
 * Beyond Target setting governs them the same way. Threshold tracks run between
 * their own bounds, which may sit below zero — that is the whole point of the
 * mode, so the shared zero floor does not apply to them.
 *
 * @param {Track} track
 ...
 *
 * Deliberately posts no chat card, for the same reason {@link setTrackThresholds}
 * does not: naming the milestones is not the same event as the track reaching
 * one, and a GM tidying up descriptions mid-session should not fire "the gate
 * falls" at the table. The current step is recomputed during sanitization, so
 * new labels take effect at once.
 *
 * @param {string} id
 * @param {any[]} labels
 * @returns {Promise<Track|null>}
 */
export async function setTrackSteps(id, labels) {
  if (!assertGM()) return null;
  const current = getTracks();
  const track = current.find((t) => t.id === id);
  if (!track) {
  ...
  const clean = sanitizeSteps(labels);
  const requested = Array.isArray(labels) ? labels.length : 0;
  if (requested > clean.length) {
    // Silently losing a label the GM typed would look like the editor dropped it
    // at random, so name the reasons it can happen.
    ui.notifications.warn(
...
/**
 * Flip whether this track announces reaching a labelled step in chat. Applies
 * immediately; the world setting "Post Progress to Chat" still gates every card,
 * and each label can opt out of announcing on its own.
 * @param {string} id
 * @returns {Promise<Track|null>}
 */
...
/**
 * Describe what a step track's move ran into, or null when it ran into nothing.
 *
 * Unlike a band crossing, this compares *values*, not a stored id: a step label
 * is an event on one exact number, so "did the value land on a label?" and "did
 * it travel over one?" are the only two questions there are. A track that moves
 * from 4 to 5 with labels at 3 and 6 has done neither, which is the ordinary
 * case and is what the null return means.
 *
 * `passed` lists the labels strictly between the two values, in the order they
 * were actually travelled, so a jump that clears three milestones names all of
 * them rather than swallowing the two it did not land on. The endpoints are
 * excluded by construction: the label under the old value was announced when the
 * track arrived there, and the label under the new one is `entered`.
 *
 * @param {Track} track    Stored state after the change.
 * @param {Track} previous Stored state before it.
 * @returns {{entered: StepLabel|null, passed: StepLabel[], direction: 1|-1}|null}
 */
export function describeStepCrossing(track, previous) {
  ...
  const entered = track.steps.find((label) => label.id === track.step) ?? null;
  const direction = to > from ? 1 : -1;

  const lo = Math.min(from, to);
  const hi = Math.max(from, to);
  const passed = track.steps.filter((label) => label.value > lo && label.value < hi);
  // Read them in the order they were actually crossed.
  if (direction < 0) passed.reverse();

  if (!entered && !passed.length) return null;
  return { entered, passed, direction };
}
 ...
 *    reached label's own `announce`, does the same for a step track reaching a
 *    labelled step. A track is in one mode, so only one of the two can apply.
 *
 * A change that trips both 2 and 3 posts one card carrying both, not two cards.
 * A plain progress track can never trip 3, so its behaviour is unchanged.
 *
...
async function postUpdateCard(track, previous, reason) {
  ...
  const announceStep =
    ...
    // A move that only travelled over labels has no reached label to consult,
    // so the track-level toggle is the only gate it can answer to — the same
    // arrangement as a drop below the lowest rung above.
    (stepCrossing.entered ? stepCrossing.entered.announce !== false : true);

  if (!announceChange && !announceCrossing && !announceStep) return;
  ...
  try {
    const content = await foundry.applications.handlebars.renderTemplate(
      `modules/${MODULE_ID}/templates/${template}`,
      {
        ...
        ...(stepped ? stepCardContext(track, stepCrossing, announceStep) : {})
      }
    );
...
/**
 * The step-specific half of a chat card's context.
 *
 * `showCrossing` is passed in rather than derived from `crossing` for the same
 * reason as above: a step the GM muted still has to render the resulting state
 * — the card says where the track now stands, it just does not make an
 * announcement of having arrived.
 *
 * @param {Track} track
...
function stepCardContext(track, crossing, showCrossing) {
  const step = resolveStep(track.current, track.steps);

  return {
    // `stepped`, matching the flag `buildStepView` sets, so one template idiom
    // reads the same on the card as it does on the HUD. The name avoids `steps`
    // because that is the label array itself — which the HUD and panel spread
    // into the very same context, and which is non-empty on a progress track
    // that used to be a step track.
    stepped: true,
    stepLabel: step ? stepDisplayName(step) : "",
    stepDescription: step?.description ?? "",
    crossing: showCrossing && crossing
      ? {
          rising: crossing.direction > 0,
          reached: crossing.entered ? stepDisplayName(crossing.entered) : "",
          passed: crossing.passed.map((label) => stepDisplayName(label))
        }
      : null
  };
}
